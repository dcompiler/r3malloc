...
pub struct DescriptorNode<'a> {
    maybe_desc: Option<Pointer<Descriptor<'a>>>,
    counter: u32
}
...
fn main() {
    println!("Hello World!");
    let HEAP: Pointer<ProcHeap> = Pointer::new(ProcHeap { partial_list: DescriptorNode { maybe_desc: None, counter: 0 } });

    let dn1 = Pointer::new(Descriptor { value: 1, next_partial: DescriptorNode { maybe_desc: None, counter: 0 }, heap: HEAP.clone() });

    let dn2 = Pointer::new(Descriptor { value: 2, next_partial: DescriptorNode { maybe_desc: None, counter: 0 }, heap: HEAP.clone() });

    HEAP.borrow().print();

    heap_push_partial(dn1);
    HEAP.borrow().print();

    heap_push_partial(dn2);
    HEAP.borrow().print();

    heap_pop_partial(&mut HEAP.borrow_mut());
    HEAP.borrow().print();
